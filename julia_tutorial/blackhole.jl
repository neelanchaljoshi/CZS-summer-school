...
N_HIDDEN = 32
NN = Lux.Chain((x) -> [cos(x[1])], # cos.(x),
    Lux.Dense(1, N_HIDDEN, cos),
    Lux.Dense(N_HIDDEN, N_HIDDEN, cos),
    Lux.Dense(N_HIDDEN, 2))
p, st = Lux.setup(rng, NN)
NN_params = ComponentArray{Float64}(p) .* 0
...
# solution = solve(prob, RK4(), saveat=tsteps, dt=dt, adaptive=false)
...
# basis = Basis(b, u);
# full_problem = ContinuousDataDrivenProblem(soln, t)
# # ideal_problem = DirectDataDrivenProblem(X̂, Ȳ)
# nn_problem = DirectDataDrivenProblem(pred_orbit, true_orbit)
# λ = exp10.(-3:0.01:3)
# opt = ADMM(λ)
